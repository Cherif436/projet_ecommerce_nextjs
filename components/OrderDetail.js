        ...
        <>
        {
            ...
            <div key={order._id} style={{margin: '20px auto'}} className="row justify-content-around">
                ...
                <div className="text-uppercase my-3" style={{maxWidth: '600px'}}>
                    <h2 className="text-break">Commande {order._id}</h2>
                    ...
                    <div className="mt-4 text-secondary">
                        ...
                        d-flex justify-content-between align-items-center`} role="alert">
                            {
                                order.delivered ? `Delivré le ${order.updatedAt}` : 'Non Delivrer'
                            }
                            {
                        ...
                        <h3>Paiment</h3>
                        {
                            order.method && <h6>Methode: <em>{order.method}</em></h6>
                        }
                        
                        {
                            order.paymentId && <p>PaiementId: <em>{order.paymentId}</em></p>
                        }
                        
                        <div className={`alert ${order.paid ? 'alert-success' : 'alert-danger'}
                        d-flex justify-content-between align-items-center`} role="alert">
                            {
                                order.paid ? `Payer le ${order.dateOfPayment}` : 'Non Payer'
                            }
